...
for x in tqdm(files, total=len(files)):
    ...
    cfts_record["resolver"] = (
        f"https://thun-korrespondenz.acdh.oeaw.ac.at/{record['id']}.html"
    )
    cfts_record["id"] = record["id"]
    record["title"] = " ".join(
        " ".join(doc.any_xpath('.//tei:title[@type="label"]//text()')).split()
    )
    ...
    record["persons"] = [
        ...
        for x in doc.any_xpath(".//tei:person[@xml:id]/tei:persName")
    ]
    cfts_record["persons"] = record["persons"]
    record["places"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:place[@xml:id]/tei:placeName")
    ]
    cfts_record["places"] = record["places"]
    record["orgs"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:org[@xml:id]/tei:orgName")
    ]
    cfts_record["orgs"] = record["orgs"]
    record["keywords"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:index/tei:term")
    ]
